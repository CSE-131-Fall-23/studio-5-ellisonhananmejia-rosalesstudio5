...
public class Methods {
	...
	public static String substituteAll(String source, char target, String replacement) {
		...
		for(int i= 0; i <source.length(); i++) {
			if(source.charAt(i) == target) { //if target is found, replace with replacement string
				result += replacement;
			}
			else { 
				result += source.charAt(i); //else if target is not found, keep the character the same
			}
		}
